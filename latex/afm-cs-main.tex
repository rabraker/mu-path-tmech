...
We scanned 5 micron by 5 micron images using standard raster scans with 512, 128 and 64 lines. The raster scans with 128 and 64 lines were binned horizontally to first produce 128 by 512 and 64 by 512 images. We then interpolated these images vertically to produce 512 by 512 pixel images.

We also took $\mu$-path scans with densities at 12.5\%, 15\% and 25\% and reconstructed 512 by 512 pixel images.
For all CS scans, the $\mu$-paths are 500 nm long, as this allows us to work around the lack of a $Z$-axis sensor. This yields a $\mu$-path size of 52 pixels.
...
For the raster scans, we took scans at 1.0~Hz, 2.5~Hz, 5.0~Hz, 8.0~Hz and
10~Hz.  The CS scans we taken with scan velocities equivalent to raster scans of 1.0~Hz,
2.0~Hz, 5.0~Hz and 8.0~Hz with sampling densities of 10~\% and 15~\%.


The CS scans were reconstructed with BPVV. We set
$\sigma = 0.1$, $\mu_f=1e-5$, $\alpha_v=0.75$ and $\alpha_h=0.0$. The data remained scaled in terms of volts.

For the raster scans, we discard data from the re-trace and divided the
remaining data for each scan line 512 bins based on the \xc sensor
...
\begin{figure*}
  \includesvg[width=0.95\linewidth]{figures/cs_raster_images_6-5-2019.svg}
...
\begin{figure*}[t!]
  \includesvg[width=1\textwidth]{figures/cs_raster_pixel_rows_6-5-2019.svg}
...
Thus, we conclude that $\mu$-path based sub-sampling is only beneficial when there is an exigent reason to scan slowly. This may be the case for delicate specimens. We can roughly quantify this by computing TKTKTK

SEGUE TO SOMETHING LIKE RDI HERE.
...
\begin{figure*}[ht!]
  ...
  \begin{subfigure}{1\textwidth}
    \centering
    \includesvg[width=0.8\textwidth]{figures/cs_rast_time_vs_ssim.svg}
    \caption{}
    \label{fig:time_ssim}
  \end{subfigure}
  \begin{subfigure}{1\textwidth}
    \centering
    \includesvg[width=0.8\textwidth]{figures/cs_rast_time_vs_psnr.svg}
    \caption{}
    \label{fig:time_psnr}
  \end{subfigure}
  \begin{subfigure}{1\textwidth}
    \centering
    \includesvg[width=0.8\textwidth]{figures/cs_rast_damage.svg}
    \caption{}

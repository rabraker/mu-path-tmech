...
\author{Roger A. Braker, Yufan Luo, \textit{Student Member, IEEE}, Lucy Y. Pao, \textit{Fellow, IEEE}\\ and Sean B. Andersson, \textit{Senior Member, IEEE, and Member, ASME}
...
\begin{abstract}
  ...
  the relative damage index. These images are compared (across a range of scan speeds)
  to both standard raster
  scans as well as to simple subline sampling where only a subset of the
  rows of the image are acquired (followed by reconstruction).
  These experiments show that at low sampling densities and slow scan rates,
  $\mu$-path scanning achieves better image quality in less time than subline sampling.
  Conversely, for faster scan rates subline sampling is faster and, for higher sampling
  densities, achieves comparable image quality.
  % These experiments demonstrate that at slow scan
...
\item We experimentally compare (Sec. \ref{sec:results}) $\mu$-path scanning to not only full density raster scans but also to coarse raster scans which are interpolated to a full image. This is in contrast to typical subline sampling which takes scan lines at random. The clear advantage to a coarse raster scan is that its implementation only requires adjustments to the data collection and post-processing. 
  % it requires no modification to the instrument, only to the post-processing.
...
focus exclusively on BPVV. These reconstructions with BPVV took about
...
% RAB: I don't see this. Whether we disengage completely or not, RDI should remain the same.
% and is a key component to the new quality metric we introduce in Sec.~\ref{sec:rdi}.
...
\begin{figure}[t!]
  ...
  \caption{Several CS cycles. Each state is indicated by color. The deflection is shifted so its free value is zero.}
  \label{fig:cs_cycle}
\end{figure}
...
based on the work of \cite{kuhnen_modeling_2003}. The drift
compensation models the piezo creep as a simple second-order transfer
function with real poles and zeros whose inverse is part of $D_X$.
In Fig.~\ref{fig:afm_bd_dinv}, the hysteresis inversion is indicated
by $\mathcal{H}_X^{-1}[\cdot]$. We
...
worse, leading to scans which cover slightly less than the intended range.
...
and the $XY$-direction of the original piezo tube scanner is uncontrolled.
...
\begin{figure}[t!]
  ...
  \includesvg[width=0.9\columnwidth]{figures/baseline_errors_1and6.svg}
...
\end{align}
with ${I = \{k : r_{Z,k} = r_{z,s}\}}$, accumulates the deflection signal while scanning as a measure of the net force applied to the specimen  over time.
...
%leading to several inconsistencies. For example, it makes no sense that the 1~Hz, 25\% CS scan has a \emph{worse} PSNR and SSIM than the 5~Hz, 25\% CS~scan; nor does it make sense that the 8~Hz full raster scan has a worse SSIM and PSNR than the 10~Hz full raster scan.
...
%, and this is where CS seems to shine. 
As one would expect, as imaging time
decreases, the RDI for each scan type increases.
From Fig.~\ref{fig:time_damage}, we see that for a fixed RDI, CS has a
faster imaging acquisition rate. Conversely, for an acquisition time below
a given threshold, CS offers a smaller RDI than a full raster scan.
% This is not the case: e.g., compare 8Hz cs and 1 hz raster.
% Note that the CS curves are always better than that of the full raster scan.
% From Fig.~\ref{fig:time_damage}, we see that for a fixed RDI, CS has a
% faster imaging acquisition rate.
The amount of improvement is best if we need a very low RDI: for example,
if we want to hold the RDI below~2, then we have to take the 1~Hz raster scan
(512 seconds) while we can achieve the same RDI via CS in about 42 seconds with the 12.5\%
...
\subsection{Comparison to subline Sampling} \label{sec:cp_subline}
Many subline sampling papers assume that the lines to sample are
...
% other artifacts in the 64 line scan obscure the defect. It is important to note, however, that the appearance (or lack thereof) of such small features in the image is in large part a random occurrence. In our scans, the line scanner happened to pass through this region and capture the appropriate data while the $\mu$-path pattern used did not. 
% Except that it passed through this region for all of the subline scans, which did not scan exactly the same area...
...
CS appears to preserve the edges of the holes better than subline sampling,
while subline sampling has less noise in the flat areas of the specimen. 

\begin{figure*}
  \centering
  \includesvg[scale=.95]{figures/cs_raster_images_subl_6-5-2019.svg}
...
\begin{figure}
  \centering
  \includesvg[width=0.85\columnwidth]{figures/improvements.svg}
